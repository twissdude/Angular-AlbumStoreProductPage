...
try {
  ProductListComponent = require("../../app/product-list/product-list.component.ts")
...
describe("ProductListComponent", () => {
  ...
  it(`should have anchor elements that have a routerLink attribute with the correct values @product-list-anchor-tags-have-routerlink-attribute`, async(() => {
    since(
      ...
      .expect(productListComponentExists)
      .toBe(true);
    ...
    if (ProductListFixture.nativeElement.querySelectorAll("li a").length > 0) {
      since(
        "The first list item tag is missing a `routerLink` attribute with the correct value."
      )
        .expect(
          ProductListFixture.nativeElement
            .querySelectorAll("li a")[0]
            .getAttribute("ng-reflect-router-link")
        )
        .toContain("product/1");
      since(
        "The second list item tag is missing a `routerLink` attribute with the correct value."
      )
        .expect(
          ProductListFixture.nativeElement
      ...
      since(
        "The first list item tag is missing a `routerLinkActive` attribute with the correct value."
      )
        .expect(
          ProductListFixture.nativeElement
            .querySelectorAll("li a")[0]
            .getAttribute("ng-reflect-router-link-active")
        )
        .toContain("active");
      since(
        "The second list item tag is missing a `routerLinkActive` attribute with the correct value."
      )
        .expect(
          ProductListFixture.nativeElement
            .querySelectorAll("li a")[1]
